    ...
    try {
        ...
        const tripsResult = await query<WeeklyTripRow>(`
            ...
            SELECT
                ...
                s.end_date_time as actual_end_date,
                EXTRACT(DAY FROM (s.end_date_time - s.start_date_time)) as days,
                EXTRACT(EPOCH FROM (s.end_date_time - s.start_date_time)) / 3600 as hours,
        ...
        res.json({
            ...
            services: services,
            driverSummary: driverSummaryResult.rows,
            totals: {
...
export const exportWeeklyTrips = async (
    ...
    try {
        ...
        // Style headers (only table columns A-L)
        const headerRow = worksheet.getRow(1);
        headerRow.height = 25;

        // Apply styles only to table columns (A-L = 1-12)
        ...
        services.forEach((service, serviceIndex) => {
            ...
            // Format duration (days, hours, and minutes)
            const totalMinutes = Math.floor(service.hours * 60);
            ...
            const endDateTime = new Date(service.actual_end_date).toLocaleString('es-PE', {
                year: 'numeric', month: '2-digit', day: '2-digit',
                hour: '2-digit', minute: '2-digit'
            });
            ...
            const principalRow = worksheet.addRow({
                ...
                driver: service.principal_driver.name,
                reason: service.principal_driver.reason,
            ...
            // Style principal row (only table columns A-L)
            ...
            service.additional_drivers.forEach((driver) => {
                ...
                // Style additional driver rows (only table columns A-L)
                row.height = 30;
                for (let col = 1; col <= 12; col++) {
            ...
            if (totalRows > 1) {
                ...
                // Merge: Code, Client, Origin, Destination, Type, Start, End, Duration, Currency, Price
                ['A', 'B', 'E', 'F', 'G', 'H', 'I', 'J', 'K', 'L'].forEach(col => {
