...
export default function PendingServicesPage() {
  ...
  const handleAssign = async (serviceId: number) => {
    try {
      ...
      setAssigningService(fullService);
    } catch (error) {
      console.error('Error fetching service for assignment:', error);
      toast.error('No se pudo cargar la información del servicio');
    }
  };

  const handleAssignSubmit = async (serviceId: number, data: AssignResourcesPayload) => {
    try {
      await servicesService.assignResources(serviceId, data);
      toast.success(`Recursos asignados al servicio #${serviceId}`);
