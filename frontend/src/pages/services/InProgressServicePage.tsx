...
import { LogOut, User as UserIcon, ArrowLeft, PlayCircle } from 'lucide-react';
import { useAuth } from '../../context/AuthContext';
import { servicesService } from '../../services/services.service';
import type { Service } from '../../interfaces/services.interface';
import { ServiceCard } from '../../components/services/ServiceCard';
import { ServiceDetailModal } from '../../components/services/ServiceDetailModal';
import { FinishServiceModal } from '../../components/services/FinishServiceModal';
...
export default function InProgressServicesPage() {
  ...
  const [serviceToFinish, setServiceToFinish] = useState<Service | null>(null);
  ...
  const handleFinishService = async (serviceId: number) => {
    try {
      ...
      setServiceToFinish(fullService);
      setFinishModalOpen(true);
    } catch (error) {
      console.error('Error fetching service for finish:', error);
      toast.error('No se pudo cargar la información del servicio');
    }
  };

  const handleConfirmFinish = async (serviceId: number, data: any) => {
    try {
      await servicesService.changeStatus(serviceId, data);
      toast.success(`Servicio #${serviceId} finalizado correctamente`);
      loadServices();
      ...
      throw error; // Propagate to modal to handle loading state/notifications if needed
    }
  };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">Servicios en Ejecución</h1>
                <p className="text-sm text-gray-600 mt-1">
                  {loading ? 'Cargando...' : `${services.length} servicios en ruta`}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-4">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {services.map(service => (
              <ServiceCard
                ...
                service={service}
                variant="in_progress"
                onAction={canModify ? handleFinishService : undefined}
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-12 text-center">
            <div className="max-w-md mx-auto">
              ...
              <h3 className="text-lg font-semibold text-gray-900 mb-2">No hay servicios en ejecución</h3>
              <p className="text-gray-600">Todas las unidades se encuentran disponibles o pendientes de salida.</p>
            </div>
          </div>
        )}
      </main>
